...
# hold fixing
#---------------------------------------------------------
  write -hierarchy -f verilog -o $::env(OUTPUT_DIR)/gl/${modname}_init.v
...
#---------------------------------------------------------
  ...
  report_timing  > $::env(OUTPUT_DIR)/reports/timing_max_slow.rpt
...
# report_timing  > timing_max_fast_${type}.rpt
  report_timing -delay min  > $::env(OUTPUT_DIR)/reports/timing_min_fast_holdcheck_${type}.rpt
...
#---------------------------------------------------------
  ...
  write_sdf $::env(OUTPUT_DIR)/sdf/counter_min.sdf
...
#---------------------------------------------------------
  ...
  set link_library   [concat  $link_library dw_foundation.sldb]
  translate
  report_timing  > $::env(OUTPUT_DIR)/reports/timing_max_slow_holdfixed_${type}.rpt
