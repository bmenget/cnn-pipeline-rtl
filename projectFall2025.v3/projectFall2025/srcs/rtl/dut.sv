...
module MAC_4x4 (
    ...
    // Stage 0: Compute partial products (bits [1:0] × 8 bits)
    logic signed [9:0] partial_prod0 [3:0][3:0];  // 2×8 = 10 bits
    logic signed [7:0] data_reg [3:0][3:0];       // Register data for subsequent stages
    logic signed [7:0] kernel_reg [3:0][3:0];     // Register kernel for subsequent stages
    logic squash_p0, capture_p0;

    // Stage 1: Add bits [3:2] contribution
    logic signed [11:0] partial_prod1 [3:0][3:0];  // accumulate up to bits [3:2]
    logic signed [7:0] data_reg1 [3:0][3:0];
    logic signed [7:0] kernel_reg1 [3:0][3:0];
    logic squash_p1, capture_p1;

    // Stage 2: Add bits [5:4] contribution
    logic signed [13:0] partial_prod2 [3:0][3:0];  // accumulate up to bits [5:4]
    logic signed [7:0] data_reg2 [3:0][3:0];
    logic signed [7:0] kernel_reg2 [3:0][3:0];
    logic squash_p2, capture_p2;

    // Stage 3: Complete multiplication with bits [7:6]
    logic signed [15:0] products_reg [3:0][3:0];
    logic squash_p3, capture_p3;

    // Stage 4: Row sums
    logic signed [19:0] sum0_p4, sum0_p4_next;
    logic signed [19:0] sum1_p4, sum1_p4_next;
    logic signed [19:0] sum2_p4, sum2_p4_next;
    logic signed [19:0] sum3_p4, sum3_p4_next;
    logic squash_p4, capture_p4;

    // Stage 5: Final sum
    logic signed [19:0] sum_p5, sum_p5_next;
    logic squash_p5, capture_p5;
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            for (int r = 0; r < 4; r++) begin
                for (int c = 0; c < 4; c++) begin
                    partial_prod1[r][c] <= 12'sd0;
                    data_reg1[r][c] <= 8'sd0;
                    kernel_reg1[r][c] <= 8'sd0;
                end
            end
            squash_p1 <= 1'b0;
            capture_p1 <= 1'b0;
            ...
            for (int r = 0; r < 4; r++) begin
                for (int c = 0; c < 4; c++) begin
                    partial_prod2[r][c] <= 14'sd0;
                    data_reg2[r][c] <= 8'sd0;
                    kernel_reg2[r][c] <= 8'sd0;
                end
            end
            squash_p2 <= 1'b0;
            capture_p2 <= 1'b0;
            ...
            // Stage 0: Multiply bits [1:0] and register inputs
            for (int r = 0; r < 4; r++) begin
                for (int c = 0; c < 4; c++) begin
                    partial_prod0[r][c] <= $signed({1'b0, data_in[r][c][1:0]}) * kernel_in[r][c];
            ...
            for (int r = 0; r < 4; r++) begin
                for (int c = 0; c < 4; c++) begin
                    partial_prod1[r][c] <= $signed(partial_prod0[r][c]) + 
                                           ($signed({1'b0, data_reg[r][c][3:2]}) * kernel_reg[r][c] <<< 2);
                    data_reg1[r][c] <= data_reg[r][c];
                    kernel_reg1[r][c] <= kernel_reg[r][c];
                end
            end
            squash_p1 <= squash_p0;
            capture_p1 <= capture_p0;

            // Stage 2: Add bits [5:4] contribution
            for (int r = 0; r < 4; r++) begin
                for (int c = 0; c < 4; c++) begin
                    partial_prod2[r][c] <= $signed(partial_prod1[r][c]) + 
                                           ($signed({1'b0, data_reg1[r][c][5:4]}) * kernel_reg1[r][c] <<< 4);
                    data_reg2[r][c] <= data_reg1[r][c];
                    kernel_reg2[r][c] <= kernel_reg1[r][c];
                end
            end
            squash_p2 <= squash_p1;
            capture_p2 <= capture_p1;

            // Stage 3: Complete multiplication with bits [7:6] (signed)
            for (int r = 0; r < 4; r++) begin
                for (int c = 0; c < 4; c++) begin
                    // Sign-extend bits [7:6] for proper signed arithmetic
                    products_reg[r][c] <= $signed(partial_prod2[r][c]) + 
                                          ($signed({{6{data_reg2[r][c][7]}}, data_reg2[r][c][7:6]}) * kernel_reg2[r][c] <<< 6);
                end
            end
            squash_p3 <= squash_p2;
            ...
            // Stage 4: Row sums
            sum0_p4 <= sum0_p4_next;
            sum1_p4 <= sum1_p4_next;
            sum2_p4 <= sum2_p4_next;
            sum3_p4 <= sum3_p4_next;
            squash_p4 <= squash_p3;
            capture_p4 <= capture_p3;

            // Stage 5: Final sum
            sum_p5 <= sum_p5_next;
            squash_p5 <= squash_p4;
            capture_p5 <= capture_p4;
        end
    end

    always_comb begin
        // Stage 4: Row sums (balanced tree)
        sum0_p4_next = (products_reg[0][0] + products_reg[0][1]) + (products_reg[0][2] + products_reg[0][3]);
        sum1_p4_next = (products_reg[1][0] + products_reg[1][1]) + (products_reg[1][2] + products_reg[1][3]);
        sum2_p4_next = (products_reg[2][0] + products_reg[2][1]) + (products_reg[2][2] + products_reg[2][3]);
        sum3_p4_next = (products_reg[3][0] + products_reg[3][1]) + (products_reg[3][2] + products_reg[3][3]);

        // Stage 5: Final sum
        sum_p5_next = (sum0_p4 + sum1_p4) + (sum2_p4 + sum3_p4);

        // Saturation logic (combinational)
        if (sum_p5 > 20'sd127) begin
            mac_out = 8'sd127;
        end else if (sum_p5 < -20'sd128) begin
            mac_out = -8'sd128;
        end else begin
            mac_out = sum_p5[7:0];
        end
    ...
    assign squash_p = squash_p5;
    assign capture_p = capture_p5;
endmodule
